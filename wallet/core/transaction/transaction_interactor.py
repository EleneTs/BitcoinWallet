...
from wallet.core.transaction.transaction import CreateTransactionRequest, TransactionResponse, TransactionInfo, \
    TransactionListResponse, ITransaction
...
class TransactionInteractor:
    ...
    def create_transaction(self, request: CreateTransactionRequest):
        ...
        if wallet_from_user.status_code != HTTPStatus.OK or wallet_to_user.status_code != HTTPStatus.OK or \
                not api_user or api_user.get_user_id() != wallet_from_user.wallet_owner.get_user_id():
            return TransactionResponse(
                status_code=HTTPStatus.BAD_REQUEST, message="Invalid credentials"
            )
        if request.amount > self.wallet_repository.fetch_wallet(request.wallet_from).balance:
            return TransactionResponse(
                status_code=HTTPStatus.BAD_REQUEST, message="Not enough balance"
            )
        commission_fee: float = 0.0

        if wallet_from_user.wallet_owner.get_user_id() == wallet_to_user.wallet_owner.get_user_id():
            # no commission
            self.wallet_repository.make_transaction(request.wallet_from, -request.amount)
        ...
        transaction_id = self.transaction_repository.create_transaction(request.wallet_from,
                                                                        request.wallet_to, request.amount)
    ...
    def get_user_transactions(self, api_key: str) -> TransactionListResponse:
        ...
        user_wallets = self.wallet_repository.get_user_wallets_address(user.get_user_id())
        for wallet_address in user_wallets:
            wallet_transactions = self.transaction_repository.fetch_transactions(wallet_address)
    ...
    def get_wallet_transactions(self, wallet_address: str, api_key: str) -> TransactionListResponse:
